export const validate = (input) => {
    ...
    if (input.name.length > 40) err.name = "That's not a name, it's a poem. Shorten it pls";
    
    if(input.minHeight.length > 0){
        if (input.minHeight < 5) err.minHeight = "Come on, I've seen cockroaches larger than 5cm...";
        else if (isNaN(input.minHeight)) err.minHeight = "Minimum height must be a number.";
    }

    if (input.maxHeight > 70) err.maxHeight = "Is that a dog or Godzilla?";
    ...
    if(input.minHeight.length > 0){
        if (input.minWeight < 1) err.minWeight = "If it weighs less than 1 kg, it's a mouse.";
        else if (isNaN(input.minWeight)) err.minWeight = "Minimum weight must be a number.";
    }

    if (input.maxWeight > 120) err.maxWeight = "Stop bluffing, must be lighter than 120kg.";
